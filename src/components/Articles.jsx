...
const Articles = ({ setIsLoading, scrollUp }) => {
  ...
  const handleVote = () => {
    if (logSuccess) {
      if (voted) {
        // Remove
        articleVote(article_id, -1).catch((err) => {
          if (err) {
            setErr(true);
            setTimeout(() => {
              setErr(false);
  ...
  return (
    <div className="flex flex-col items-center p-5 pt-24 text-center gap-2 articleSingle">
      ...
      <div className="p-5 flex gap-10">
        <div className="flex gap-1">
          <button onClick={() => setCommentClick(!commentClick)}>
            {" "}
            {commentClick ? (
        ...
        <button onClick={handleVote}>
          {!voted ? (
            <div className="flex gap-1 pb-1.5">
              ...
              <p className="votes">{articleId.votes}</p>
            </div>
          ) : (
            <div className="flex gap-1 pb-1.5">
              {" "}
      ...
      {commentClick ? (
        ...
        <div className="flex flex-col items-center">
          ...
          {error || commentErr ? (
            <h3 className=" text-red-600">Error! Please try again</h3>
          ) : (
            ""
          )}
          {!commentErr && success ? (
            <h3 className=" text-green-600">Comment posted!</h3>
          ) : (
            ""
          )}
          ...
          {commentsId.map((comments) => {
            return (
              <div
                ...
                className="p-4 flex flex-col items-center gap-4 comments"
              >
                <hr />
          ...
          <button>
            <span
              className="material-symbols-outlined p-2 upArrow"
              onClick={scrollUp}
            >
              arrow_upward
            </span>
